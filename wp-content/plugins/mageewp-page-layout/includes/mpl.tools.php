...
class mpl_tools {
	...
	public static function get_featured_image( $post, $thumbnail = 'single-post-thumbnail' , $first = true ) {
		...
		if( empty($featured) )
		{
			...
			else return MPL_URL.'/assets/images/default.jpg';
		}
		return $featured[0];
	...
	public static function get_first_image( $content, $id = null ) {
		...
		if(empty($first_img)){
			...
			else $first_img = MPL_URL.'/assets/images/default.jpg';
		}
	...
	public static function createImageSize( $source, $attr ){
		...
		if (strpos($source, MPL_SITE) === false || $source == MPL_URL.'/assets/images/get_start.jpg')
			return $source;
		...
		if( strpos( $source, strrev( 'images/default.jpg' ) ) === 0 ){
			...
			$file = str_replace( array( untrailingslashit( site_url() ).'/', '\\', '/' ), array( ABSPATH, MDS, MDS ), strrev( $source ) );
			
			$_return = strrev( substr( $source, 0, $st+1 ).strrev($attr).substr( $source, $st+1 ) );
			$__return = str_replace( array( untrailingslashit( site_url() ).'/', '\\', '/' ), array( ABSPATH, MDS, MDS ), $_return );
	...
	public static function get_posts( $atts ){
		...
		$atts = shortcode_atts( array(
			...
			'gap'     		     	 => '0',
			'post_type'              => 'mpl-testimonials',
			'taxonomy'               => 'mpl-testimonials-category',
			'tax_term'               => false,
			'order'                  => 'desc',
			'filter'			     => 'No',
			'margin'			     => 'Yes',
	...
	public static function get_cf7_names() {
		...
		if ( $cf7_list ) {
			...
			$cf7_val[] = __( 'Select Contact Form', 'mageewp-page-layout' );
			...
			$cf7_val[0] = __( 'No contact forms found', 'mageewp-page-layout' );
