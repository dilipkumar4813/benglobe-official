...
class mpl_optimized {
	...
	function __construct(){
		...
		$this->surl = trailingslashit(str_replace(array('http://', 'https://'), "", MPL_SITE));
		
		if (!is_admin()) {
			
			$case_get = false;
			if (isset($_GET['mpl_optimized_action'])) {
				$action = $_GET['mpl_optimized_action'];
				switch ($action) {
					case 'gethtml': 
	...
	public function in_footer() {
		...
		$settings = get_option('mpl_optimized', true);
		if (!mpl_is_using() || !$mpl->is($settings, array('enable'), 'on'))
			return;
			
		$mpl_meta = get_post_meta ($post->ID , 'mpl_data', true);
		
		if (isset($mpl_meta['optimized']) && $mpl_meta['optimized'] == 'deactive'){
			return;
		}
		
		if ($settings['global'] != 1 && (!isset($mpl_meta['optimized'])))
			return;
	...
	private function get_html ($url = '') {
		...
		if (strpos($url, '?') === false)
			$url .= '?mpl_optimized_action=gethtml&nightly=cache';
		else $url .= '&mpl_optimized_action=gethtml&nightly=cache';
...
EOD;
		...
		if (strpos($htcontent, '# BEGIN MPL Optimized') !== false && 
			strpos($htcontent, '# END MPL Optimized') !== false &&
			(
				($advanced == 0 && strpos($htcontent, '# MPL Gzip') === false) || 
				($advanced == 1 && strpos($htcontent, '# MPL Gzip') !== false)
			)
		)return true;
		
		if (($advanced == 0 && strpos($htcontent, '# MPL Gzip') !== false) || 
			($advanced == 1 && strpos($htcontent, '# MPL Gzip') === false)) {
				$htcontent = preg_replace('/# BEGIN MPL Optimized(.*)# END MPL Optimized/Uis', '', $htcontent);
			}
		...
		if(ABSPATH == "//"){
			$mplhtaccess .= "RewriteCond %{DOCUMENT_ROOT}/optimized/$1/index.html -f"."\n";
		}else{
			$mplhtaccess .= 'RewriteCond %{DOCUMENT_ROOT}/optimized/$1/index.html -f [or]'."\n";
			$mplhtaccess .= 'RewriteCond '.ABSPATH.'optimized'.$path.'/$1/index.html -f'."\n";
		}	
		
		$mplhtaccess .= 'RewriteRule ^(.*) "'.$path.'/optimized'.$path.'/$1/index.html" [L]'."\n".'</IfModule>'."\n";

		$mplhtaccess .= '<FilesMatch "\.(html|htm)$">'."\n".
...
EOD;
		...
		$mplhtaccess .= "\n".'# END MPL Optimized'."\n";
		if (!file_put_contents(ABSPATH.'.htaccess', $mplhtaccess.$htcontent) && !is_file(ABSPATH.'.htaccess'))
			return false;
	...
	private function create_combined ($links = array(), $type = 'js') {
		...
		foreach ($links as $link) {
			...
			if (strpos($link, $this->surl) !== false) {
				$path = str_replace(array($this->surl, '/'), array(ABSPATH, MDS), $link);
				$path = parse_url($path);
	...
	private function minify_js($input = '') {
		...
		require_once('mpl.vendors.jsmin.php');
	...
	private function render_path_name ($url = '') {
		...
		return untrailingslashit($path).MDS.$name;
	...
	public function delete_cache ($url = 'all') {
		...
		if ($url == 'all') {
			return mpl_remove_dir(ABSPATH.'optimized');
		}else{
