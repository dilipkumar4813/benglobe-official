...
header: {
    ...
    init: function () {
        //Header data
        if (mpl.maps['mpl_header'] === undefined)
            return false;

        var map = $().extend({}, mpl.maps['_std']);
        mpl.front.header.params = $().extend(map, mpl.maps['mpl_header']);
 
        if (mpl_top_bar_header === undefined || mpl_top_bar_header === '')
            return ;

        mpl.front.header.storage = JSON.parse(mpl_top_bar_header);
    },
    backbone: {
        save: function(pop) {
            //radiotabs每次选择时将未选中的input元素加上mpl-no-param，radiotabs加载后如果没有手动选择过，则可能
            //会导致mpl-no-param不完整（因为radiotabs子元素渲染在radiotabs自身后面完成，初始化时模拟的click事件
            //无法为子元素加上mpl-no-param)。为防止这种情况再触发一次click事件。
            //var slug = pop.find('.field-radiotabs .radiotabs-nav .active').attr('data-tab');
            //pop.find('.field-radiotabs .mpl-radiotabs-rows li[data-tab="' + slug + '"]').trigger('click');
            pop.find('.field-base-header_layout .mpl-radio-image-field-body input.m-p-rela').each(function(){
                if($(this).attr('checked')) 
                    $(this).trigger('change');
            });

            mpl.front.header.storage = mpl.tools.getFormGroupData(pop);
        },
        settings: function (e, atts) {
            ...
            for (var tab in mpl.front.header.storage) {
                for (var n in mpl.front.header.storage[tab]) {
                    if (typeof(mpl.front.header.storage[tab][n]) === 'object')
                        data[n] = JSON.stringify(mpl.front.header.storage[tab][n]);
                    else
                        data[n] = mpl.front.header.storage[tab][n];
                }
            }
            ...
            var map = $().extend({}, mpl.front.header.params);

            if (map.title === undefined)
                map.title = 'Top bar & Header Settings';
            var attz = {
                title: map.title,
                width: map.pop_width,
                scrollBack: false,
                ...
                storage: data,
                params: map,
                class: 'mpl_header_wrpop mpl-elm-settings-popup',
            };
            if (atts !== undefined)
            ...
            pop.data({ model: mid, callback: mpl.front.header.backbone.save });
...
init: function () {
	...
	mpl.trigger({
		...
		events: {
			'.header:click': mpl.front.header.backbone.settings,
...
submit: function () {
	...
	/* Top bar & Header data */
	var header_setting = {};
	header_setting = JSON.stringify(mpl.front.header.storage);
